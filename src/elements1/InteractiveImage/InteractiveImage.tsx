...
import { Animated, StyleSheet, Dimensions, View, PanResponder, TouchableWithoutFeedback, TouchableHighlight, Modal, Text } from 'react-native';
...
class InteractiveImage extends Component<InteractiveImageProps> {

    private popupRef: any;
    private trueFunction: any;
    private falseFunction: any;
    private previousDistanceX: any;
    private previousDistanceY: any;
    private imageRef: any = createRef();
    private popupContentRef: any = createRef();
    private scale: any;
    ...
    constructor(props: any) {
        super(props)
        this.popupRef = (ref: any) => this.popupRef = ref
        ...
        // this.imageRef = (ref: any) => this.imageRef = ref;
        // this.popupContentRef = (ref: any) => this.popupContentRef = ref;
    }
    ...
    onPanResponderGrant(e: any, s: any) {
        this.previousDistanceX = 0
        this.previousDistanceY = 0
    ...
    onPanResponderMove(e: any, s: any) {
        // zoom
        if (s.numberActiveTouches === 2) {
            const dx = Math.abs(e.nativeEvent.touches[0].pageX - e.nativeEvent.touches[1].pageX)
            const dy = Math.abs(e.nativeEvent.touches[0].pageY - e.nativeEvent.touches[1].pageY)
    ...
    onPanResponderRelease(e: any, state: any) {
    }
    ...
    normalizeAnnotation(annotation: any) {
        if (!annotation)
            return
    ...
    getAnnotation(x: any, y: any) {
        let match: any;
        this.props.annotations && this.props.annotations.every((annotation: any) => {
            const { x1, x2, y1, y2 }: any = this.normalizeAnnotation(annotation)
    ...
    onImagePress(e: any) {
        const { nativeEvent: { locationX = 0, locationY = 0, pageX, pageY } = {} }: any = e
    ...
    renderTouchpoints() {
        if (this.state.isZooming || this.state.inZoomedState)
            return null
        return this.props.annotations && this.props.annotations.map((annotation: any, index: number) => {
            const style = {
                position: 'absolute',
